...
def plot_torus(elev=30, azim=45):
    ...
    for s in [1, 2]:
        ...
        z_stream = r * np.sin(0)
        ax.plot(x_stream, y_stream, z_stream, linewidth=2)
        ...
        for j in range(nodes_per_stream):
            ...
            z_node = r * np.sin(0)
            ax.scatter(x_node, y_node, z_node, s=50)
            ax.text(x_node, y_node, z_node + 0.05, str(j), fontsize=8)
    ...
    z_arc = r * np.sin(0)
    ax.plot(x_arc, y_arc, z_arc, linewidth=3)
    ...
    z_mid = r * np.sin(0)
    ax.text(x_mid, y_mid, z_mid + 0.1, 'Δ', fontsize=12)

    # Major radius arrow (R)
    ax.quiver(0, 0, 0, R, 0, 0, linewidth=2, arrow_length_ratio=0.1)
    ax.text(R / 2, 0, 0, 'R', fontsize=12)

    # Minor radius arrow (r)
    tube_center = np.array([(R + r * np.cos(0)) * np.cos(0),
                            (R + r * np.cos(0)) * np.sin(0),
                            r * np.sin(0)])
    ax.quiver(tube_center[0], tube_center[1], tube_center[2], r, 0, 0,
              linewidth=2, arrow_length_ratio=0.1)
    ax.text(tube_center[0] + r / 2, tube_center[1], tube_center[2], 'r', fontsize=12)

    # Labels
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Toroid with Streams, Nodes & Phase Offset')
    ax.view_init(elev=elev, azim=azim)
    plt.show()

# Interactive sliders for elevation and azimuth
interact(
    plot_torus,
    elev=FloatSlider(min=0, max=90, step=1, value=30, description='Elev'),
    azim=FloatSlider(min=0, max=360, step=1, value=45, description='Azim')
)
